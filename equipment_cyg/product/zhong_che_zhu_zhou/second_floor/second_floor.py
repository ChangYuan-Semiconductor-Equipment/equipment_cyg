...
class SecondFloor(Controller):  # pylint: disable=R0901
    ...
    def save_current_recipe_local(self):
        """保存当前的配方id和name."""
        self.config["current_recipe_name"] = self.get_sv_value_with_name("current_recipe_name")
    ...
    def _on_rcmd_pp_select(self, recipe_name):
        ...
        Args:
            recipe_name (str): 要切换的配方id_name.
        """
        self.pp_select_fail = False
        for plc_recipe_id_name, _ in self.recipes.items():
            if recipe_name in plc_recipe_id_name:
                recipe_id, recipe_name = plc_recipe_id_name.split("_")
        ...
        if self.pp_select_fail is False:
            self.set_sv_value_with_name("pp_select_state", 2)
